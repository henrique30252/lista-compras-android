...
def input_esc(prompt):
    valor = input(prompt)
    return valor.upper()  # Armazena sempre em caixa alta
...
def cadastrar_produto_apk(nome, marca, quantidade_embalagem, codigo_barras, categoria_id=None, subcategoria_id=None, imagem=None):
    try:
        conn = conectar()
        cursor = conn.cursor()
        cursor.execute('''
...
def listar_produtos_apk():
    try:
        conn = conectar()
        cursor = conn.cursor()
        cursor.execute("SELECT nome, marca, quantidade_embalagem, codigo_barras, categoria_id FROM produtos")
        produtos = cursor.fetchall()
        conn.close()
        return produtos
    except Exception as e:
...
def visualizar_lista():
    try:
        # Exibe as listas cadastradas antes de pedir o ID
        listar_listas()
        lista_id = input_esc("ID da lista: ")
        lista_id = int(lista_id)
        conn = conectar()
        cursor = conn.cursor()
        # Busca todos os itens da lista com categoria
        cursor.execute('''
...
def adicionar_item_lista():
    try:
        ...
        lista_id = input_esc("ID da lista: ")
        lista_id = int(lista_id)
        # Exibe os itens já adicionados à lista selecionada
...
def criar_carrinho():
    try:
        ...
        supermercado_id = int(supermercado_id)
        # Associar uma lista de compras ao carrinho
        listar_listas()
...
def visualizar_carrinho(carrinho_id=None):
    try:
        # Exibe os carrinhos existentes antes de pedir o ID
        listar_carrinhos()
...
def adicionar_item_carrinho(carrinho_id=None, produto_id=None, quantidade=None, preco_unit=None):
    try:
        ...
        conn = conectar()
        cursor = conn.cursor()
        # Recupera a lista associada ao carrinho
        ...
        # Exibe os itens da lista associada
        while True:
            ...
            if produto_id == "0":
                ...
                break
            produto_id = int(produto_id)
            # Busca quantidade sugerida da lista
            ...
            # Alimenta o histórico de preços
            ...
            # Subtrai a quantidade do produto na lista associada
...
def alterar_item_carrinho(item_id=None, quantidade=None, preco_unit=None):
    try:
        ...
        # Recupera dados do item do carrinho
        ...
        # Recupera quantidade do produto na lista associada
        ...
        # Atualiza ou exclui item do carrinho
        if nova_quantidade == 0:
...
def listar_historico_precos(produto_id=None):
    try:
        ...
        conn.close()
        print("\n--- Histórico de Preços ---")
        # Organiza por categoria, produto e supermercado
        categorias_dict = {}
        for h in historico:
...
def listar_categorias_e_subcategorias():
    try:
        conn = conectar()
        cursor = conn.cursor()
        # Busca todas as categorias com quantidade de produtos
        cursor.execute('''
            SELECT c.id, c.nome,
        ...
        ''')
        categorias = cursor.fetchall()
        # Busca todas as subcategorias com quantidade de produtos
        cursor.execute('''
...
def listar_categorias_apk():
    """Retorna apenas as categorias (id, nome)"""
    try:
        conn = conectar()
        cursor = conn.cursor()
        cursor.execute("SELECT id, nome FROM categorias")
...
def listar_subcategorias_apk(categoria_id):
    """Retorna apenas as subcategorias (id, nome, categoria_id)"""
    try:
        conn = conectar()
        cursor = conn.cursor()
